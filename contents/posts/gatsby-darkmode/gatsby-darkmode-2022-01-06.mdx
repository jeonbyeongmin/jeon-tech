...
console.log 는 클라이언트에서 실행되고 console.log 가 실행이 될 시점엔 이미 첫 페이지가 렌더링되어 있고 hydration까지 진행되어 localstorage 값은 잘 가져온 상태였기 때문이라 추측할 수 있습니다.

이 문제는 다양한 방법으로 해결할 수 있을 것이라 생각합니다. 많은 사람들은 이 문제를 테마 결정 단계를 렌더링 이전에 놓는 방식으로 해결하고 있었습니다. 브라우저의 렌더링 엔진이 동작하는 과정은 다음과 같습니다.
...
그렇다면 우리는 렌더링 엔진이 DOM 트리를 만들기 전에 HTML에 미리 테마를 결정해야 합니다. 어떻게 해결할 수 있을까요?

## 해결 방법

gatsby-ssr.js 파일에서 Gatsby에서 제공하는 API를 사용하여 빌드 시에 생성될 HTML에 미리 적용될 테마를 결정해줄 수 있습니다. 원리도 그다지 어렵지 않습니다. 먼저 브라우저가 파싱 도중 script 태그를 만났을 때 어떻게 되는지 이해하는 것이 중요합니다.

### 1. HTML 파싱 중에 script 태그를 만나면?

브라우저가 html을 파싱할 때 script 태그를 만나면 일반적으로 렌더를 중단하고 script를 다운받습니다. 이는 script가 DOM에 영향을 줄 수 있기 때문이죠. 그래서 보통 script 태그는 body 맨 끝에 위치 시킵니다. script를 다운받느라 렌더링을 멈추면 아무래도 사용자 경험에 좋지 않기 때문이겠죠.

### 2. 그렇다면 body 맨 앞에 script를 만들자!

body 맨 앞에 script를 작성해둘 수 있다면 어떨까요? 그리고 이 안에 body에 class를 부여하는 즉시 실행 함수를 작성한다면 미리 적용될 테마를 결정할 수 있게 하는 것이죠. 당연히 이때는 런타임이기 때문에 window 와 같은 브라우저 API도 사용할 수 있습니다.

### 3. gatsby-ssr.js

개츠비는 gatsby-ssr.js 에서 사용할 수 있는 API를 몇 개 지원합니다. 그 중 하나가 setPreBodyComponents 입니다. 말 그대로 body 앞에서 뭔가를 해줄 수 있게 해주는 API 입니다. 우리는 아래처럼 작성하여 body에 클래스 이름을 부여하고 window.\__theme에 현재 테마를 저장하고 window.\__setPreferredTheme 에 테마를 바꾸는 함수를 저장해둘 수 있습니다. 그래서 상태 관리 라이브러리가 있을 필요가 없다는 느낀 것입니다. 여기서 이미 테마의 상태를 저장해두는 변수를 만들었으니까요.
